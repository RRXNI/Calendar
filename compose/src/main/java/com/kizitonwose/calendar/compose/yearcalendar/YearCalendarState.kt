 ...
 *
 * @param startDate the initial value for [YearCalendarState.startDate]
 * @param endDate the initial value for [YearCalendarState.endDate]
 ...
 */
@Stable
class YearCalendarState internal constructor(
    startYear: Year,
    endYear: Year,
...
//            VisibleItemState(firstVisibleItemIndex = getScrollIndex(firstVisibleWeekDate) ?: 0)
    ...
    private fun getScrollIndex(date: LocalDate): Int? {
        if (date !in startDateAdjusted..endDateAdjusted) {
            Log.d("YearCalendarState", "Attempting to scroll out of range; $date")
            return null
        }
