...
internal fun LazyGridScope.CalendarMonthsGrid(
    monthCount: Int,
    ...
    monthContainer: (@Composable LazyGridItemScope.(CalendarMonth, container: @Composable () -> Unit) -> Unit)?,
) {
    items(
        count = monthCount,
        ...
        monthContainer.or(defaultMonthContainer)(month) {
            Column(
                modifier = Modifier
                    .then(if (hasContainer) Modifier.fillMaxWidth() else Modifier.fillMaxWidth())
                    .then(
                        if (fillHeight) {
                            if (hasContainer) Modifier.fillMaxHeight() else Modifier.fillMaxHeight()
                        } else {
...
private val defaultMonthContainer: (@Composable LazyGridItemScope.(CalendarMonth, container: @Composable () -> Unit) -> Unit) =
